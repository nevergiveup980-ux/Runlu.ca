(()=>{
  ...
  const CATALOG={
    fr:{
      'Latest · 5 articles':'Derniers · 5 articles',
  ...
  function ensureHealthViewLinks(){
    ...
    const entries=[
      ...
      {href:'health-view-009-recovery-margin.html',en:'View 009 · Recovery',zh:'观察 009 · 恢复余量',fr:'Regard 009 · Récupération',es:'Vista 009 · Recuperación'}
    ];
  ...
  function ensureGuanshiLinks(){
    ...
    if(['guanshi.html','guanshi-map.html'].includes(page)){
      const nav=document.querySelector('.gs-top-nav');
      if(nav&&!nav.querySelector('a[href="guanshi-tradition.html"]')){
        const link=document.createElement('a');
        link.href='guanshi-tradition.html';
        link.dataset.en='Tradition';
        link.dataset.zh='原典线';
        link.dataset.fr='Tradition';
        link.dataset.es='Tradición';
        link.textContent='Tradition';
        const mapLink=nav.querySelector('a[href="guanshi-map.html"]');
        if(mapLink)mapLink.insertAdjacentElement('afterend',link);else nav.appendChild(link)
      }
      if(page==='guanshi-map.html'){
        const head=document.querySelector('#traditional .gs-section-head > div');
        if(head&&!head.querySelector('a[href="guanshi-tradition.html"]')){
          const link=document.createElement('a');
          link.href='guanshi-tradition.html';
          link.className='gs-primary-link';
          link.dataset.en='Open the traditional source spine →';
          link.dataset.zh='进入传统原典线 →';
          link.dataset.fr='Ouvrir la trame des sources →';
          link.dataset.es='Abrir el eje de fuentes →';
