...
query RunluPulseClean($accountTag: string!, $start: Time!, $end: Time!, $host: string!) {
  viewer {
    accounts(filter: { accountTag: $accountTag }) {
      ...
      trend: rumPageloadEventsAdaptiveGroups(
        ...
        filter: { datetime_geq: $start, datetime_leq: $end, requestHost: $host, bot: 0 }
      ) { count sum { visits } dimensions { datetimeHour requestPath } }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname !== "/api/analytics") return baseWorker.fetch(request, env);
...
async function fetchAnalytics(env, days) {
  ...
  const publicPageRows = (account.pages || []).filter((row) => !isInternalOrEmbeddedPath(row.dimensions?.requestPath));
  const internalPageRows = (account.pages || []).filter((row) => isInternalOrEmbeddedPath(row.dimensions?.requestPath));
  const pages = combinePages(publicPageRows);
  const internalPages = combinePages(internalPageRows);
  const internalPageViews = internalPages.reduce((sum, item) => sum + item.views, 0);
  ...
  const internalBreakdown = internalPages.length
    ? internalPages.map((item) => `${item.name}: ${item.views}`).join("; ")
    : "none";
  ...
  return {
    meta: {
      ...
      botsExcluded: true,
      readOnly: true,
      publicViewPolicy: "Known /flooring paths are treated as internal or embedded system views."
    },
    summary: {
      pageViews,
      rawPageViews,
    ...
    devices,
    referrers,
    note: `Public-facing page views exclude ${internalPageViews} known internal/embedded Flooring OS views from ${rawPageViews} raw runlu.ca page views. Internal breakdown: ${internalBreakdown}. Visits, country, browser and device panels remain Cloudflare host-level signals, so they may still include internal system sessions. Possible external traffic remains a heuristic and is not visitor identification.`
  };
}
...
function aggregateHourlyTrend(rows, end, timeZone, days) {
  ...
  for (const row of rows) {
    if (isInternalOrEmbeddedPath(row.dimensions?.requestPath)) continue;
...
function startOfLocalDay(end, timeZone, daysBack) {
  ...
  return zonedTimeToUtc(target.getUTCFullYear(), target.getUTCMonth() + 1, target.getUTCDate(), 0, 0, 0, timeZone);
}
