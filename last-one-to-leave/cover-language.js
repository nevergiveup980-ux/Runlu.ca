(()=>{
  const BOOKS={
    ...
    night:{selector:'a.field-card[href="night-has-warmth/"]',covers:{en:'/night-has-warmth/cover.webp',zh:'/night-has-warmth/cover.webp',fr:'/night-has-warmth/cover.webp',es:'/night-has-warmth/cover.webp'},alts:{en:'Warmth in the Night cover',zh:'《夜色有温度》封面',fr:'Couverture de La chaleur de la nuit',es:'Portada de Calor en la noche'}}
  };
  ...
  function syncShelfHeading(lang){
    document.querySelectorAll('[data-en]').forEach(el=>{
      if(el.dataset.en==='Two books on the RUNLU shelf.'||el.dataset.en==='Three books on the RUNLU shelf.'){
        el.dataset.en='Three books on the RUNLU shelf.';
        el.dataset.zh='润庐书架上的三部长篇。';
        el.dataset.fr='Trois livres dans la bibliothèque RUNLU.';
        el.dataset.es='Tres libros en la biblioteca RUNLU.';
  ...
  function sync(){
    ...
    const night=document.querySelector(BOOKS.night.selector);if(night)night.querySelectorAll('[data-en]').forEach(el=>{el.textContent=el.dataset[lang]||el.dataset.en;});
