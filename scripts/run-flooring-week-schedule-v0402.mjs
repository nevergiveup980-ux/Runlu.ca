...
const path=new URL('../flooring/week-schedule-v0402.js',import.meta.url);
const source=fs.readFileSync(path,'utf8');
const ctx={globalThis:{},console};ctx.globalThis.globalThis=ctx.globalThis;
vm.createContext(ctx.globalThis);vm.runInContext(source,ctx.globalThis,{filename:'week-schedule-v0402.js'});
const A=ctx.globalThis.RUNLUWeekScheduleV0402;
...
test('Pickup folder label Awaiting Confirmation wins over pickup date bucket',()=>{
  const x=A.pickupRecordFromJob({id:'p1',jobNumber:'70',pickupFolder:'Awaiting Confirmation',pickupDate:'2026-09-20'},'2026-09-18');
  assert(x);assert.equal(x.bucket,'awaiting-confirmation')
});
test('Pickup People to Call can come from explicit call flag',()=>{
  const x=A.pickupRecordFromJob({id:'p2',jobNumber:'71',pickupDate:'2026-09-20',pickupNeedsCall:true},'2026-09-18');
  assert(x);assert.equal(x.bucket,'people-to-call')
});
test('Pickup date classifies This Month and Next Month across year boundary',()=>{
  const a=A.pickupRecordFromJob({id:'p3',jobNumber:'72',pickupDate:'2026-12-29'},'2026-12-15');
  const b=A.pickupRecordFromJob({id:'p4',jobNumber:'73',pickupDate:'2027-01-03'},'2026-12-15');
  assert.equal(a.bucket,'this-month');assert.equal(b.bucket,'next-month');assert.equal(A.nextMonthKey('2026-12-15'),'2027-01')
});
test('Pickup scheduled outside this or next month stays Other Scheduled',()=>{
  const x=A.pickupRecordFromJob({id:'p5',jobNumber:'74',pickupDate:'2027-03-10'},'2026-12-15');assert(x);assert.equal(x.bucket,'other-scheduled')
});
test('Ordinary Flooring order without pickup fields is not invented as pickup',()=>{
  const x=A.pickupRecordFromJob({id:'j1',jobNumber:'75',status:'Confirmed',installDate:'2026-09-20'},'2026-09-18');assert.equal(x,null)
});
test('Dead Files require explicit order completion and ignore installStatus alone',()=>{
  assert.equal(A.isDeadFileJob({id:'j1',status:'Completed'}),true);
  assert.equal(A.isDeadFileJob({id:'j2',orderStatus:'Closed'}),true);
  assert.equal(A.isDeadFileJob({id:'j3',installStatus:'Completed',status:'Confirmed'}),false);
  assert.equal(A.isDeadFileJob({id:'j4',completed:true}),true)
});
test('Completed order is excluded from Pickup Desk and appears in Dead Files',()=>{
  const datasets={flooringJobs:[{id:'j1',jobNumber:'76',customerName:'Done',status:'Completed',pickupFolder:'Pick-ups This Month',pickupDate:'2026-09-20'}]};
  const d=A.pickupDesk(datasets,'2026-09-18');assert.equal(d.totalPickup,0);assert.equal(d.deadCount,1);assert.equal(d.deadFiles[0].jobNumber,'76')
});
test('Pickup Desk mirrors four physical folder groups without guessing unclassified',()=>{
  const datasets={flooringJobs:[
    {id:'a',jobNumber:'80',pickupFolder:'Awaiting Confirmation'},
    {id:'b',jobNumber:'81',pickupNeedsCall:true},
    {id:'c',jobNumber:'82',pickupDate:'2026-09-28'},
    {id:'d',jobNumber:'83',pickupDate:'2026-10-05'},
    {id:'e',jobNumber:'84',pickupStatus:''}
  ]};
  const d=A.pickupDesk(datasets,'2026-09-18');
  assert.equal(d.folders['awaiting-confirmation'].length,1);assert.equal(d.folders['people-to-call'].length,1);assert.equal(d.folders['this-month'].length,1);assert.equal(d.folders['next-month'].length,1);assert.equal(d.folders.unclassified.length,1)
});
test('Dead Files reference date does not claim completion date when only install date exists',()=>{
  const x=A.deadFileRecord({id:'j1',jobNumber:'85',status:'Completed',installDate:'2026-09-10'});assert(x);assert.equal(x.archiveDate,'');assert.equal(x.referenceDate,'2026-09-10')
});
test('Morning Brief adds pickup desk sentence only when pickup-related orders exist',()=>{
  const datasets={people:[],flooringJobs:[{id:'p1',jobNumber:'86',pickupFolder:'Awaiting Confirmation'}],supplierPOs:[],warehouseOrders:[],manualEvents:[]};
  const b=A.morningBrief(datasets,[],'2026-09-18');assert(b.sentences.some(s=>s.includes('Pickup desk: 1 awaiting confirmation')));assert.equal(b.pickup.totalPickup,1)
});
