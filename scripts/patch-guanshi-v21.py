...
# Consultation controller: local baseline first, explicit local-only, cloud fallback.
...
text = text.replace(
    "privacy:'Privacy: consultation fields are processed server-side for this analysis. Fixed chart/cast/site mechanics are returned with the result. A frozen validation case remains local-first unless you export it.'",
    "privacy:'Privacy: the local baseline runs only in this browser. Consultation fields are sent server-side only when cloud enhancement is used. A frozen validation case remains local-first unless you export it.'"
)
text = text.replace(
    "privacy:'隐私说明：咨询资料会发送到服务器端用于本次分析；固定排盘、起卦和场地计算会随结果一并返回。冻结后的验证案例正文仍默认只保存在当前浏览器，除非你自己导出。'",
    "privacy:'隐私说明：本地观势基线只在当前浏览器运行；只有使用云端增强时，咨询资料才会发送到服务器端。冻结后的验证案例正文仍默认只保存在当前浏览器，除非你自己导出。'"
)
text = text.replace(
    "privacy:'Confidentialité : les données sont traitées côté serveur ; les calculs fixes sont renvoyés avec le résultat. Un cas figé reste local au navigateur sauf export.'",
    "privacy:'Confidentialité : la base locale s’exécute uniquement dans ce navigateur. Les données ne sont envoyées au serveur que pour l’analyse cloud. Un cas figé reste local sauf export.'"
)
text = text.replace(
    "privacy:'Privacidad: los datos se procesan en el servidor; los cálculos fijos vuelven con el resultado. Un caso congelado permanece local salvo exportación.'",
    "privacy:'Privacidad: la línea base local se ejecuta solo en este navegador. Los datos se envían al servidor únicamente cuando se usa el análisis en la nube. Un caso congelado permanece local salvo exportación.'"
)
...
# CI contract.
p = '.github/workflows/site-check.yml'
text = read(p)
if 'node --check guanshi-local-reasoner.js' not in text:
    text = text.replace('          node --check guanshi-knowledge.js\n', '          node --check guanshi-knowledge.js\n          node --check guanshi-local-reasoner.js\n', 1)
if 'node scripts/check-guanshi-local-reasoner.mjs' not in text:
    text = text.replace('      - name: Enforce complete four-language pages\n        run: node scripts/check-multilingual.mjs\n', '      - name: Enforce complete four-language pages\n        run: node scripts/check-multilingual.mjs\n      - name: Test GUANSHI local reasoner\n        run: node scripts/check-guanshi-local-reasoner.mjs\n', 1)
old = '''          if "_runlu_local_knowledge_context" not in consult or "GUANSHI_KB?.search" not in consult:
              raise SystemExit('GUANSHI consultation is not using the local knowledge retriever')
'''
if 'V2.1 local reasoning engine' not in text:
    if old not in text:
        raise SystemExit('site check knowledge assertion missing')
    text = text.replace(old, old + '''          if "GUANSHI_LOCAL_REASONER?.analyze" not in consult or "data-local-only" not in consult:
              raise SystemExit('GUANSHI consultation is not using the V2.1 local reasoning engine')
''', 1)
page_anchor = '''              if 'guanshi-library.html' not in text:
                  raise SystemExit(f'{page} is missing the Local Library route')
'''
if 'guanshi-local-reasoner.js?v=20260907-1' not in text:
    if page_anchor not in text:
        raise SystemExit('site check page anchor missing')
    text = text.replace(page_anchor, page_anchor + '''              if 'guanshi-local-reasoner.js?v=20260907-1' not in text or 'data-local-only' not in text:
                  raise SystemExit(f'{page} is missing the V2.1 local reasoning route')
''', 1)
text = text.replace("          print('GUANSHI V2 local knowledge + consent-controlled local library wiring verified.')", "          print('GUANSHI V2.1 local knowledge + local reasoning + consent-controlled library wiring verified.')")
write(p, text)
