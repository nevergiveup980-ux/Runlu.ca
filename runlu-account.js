(() => {
  ...
  function applyLanguage(v){lang=normalize(v);localStorage.setItem('runlu-account-language',lang);document.documentElement.lang=lang==='zh'?'zh-Hans':lang;el.languageSelect.value=lang;document.querySelectorAll('[data-i18n]').forEach(n=>{const value=t(n.dataset.i18n);if(value)n.textContent=value});syncMode();if(el.profileSummary&&!el.profileSummary.hidden)syncProfileSummary()}
  ...
  function localeName(value){const names={en:'English',zh:'中文',fr:'Français',es:'Español'};return names[normalize(value)]||names.en}
  function syncProfileSummary(){if(!el.profileSummary)return;el.profileSummaryName.textContent=el.profileName.value.trim()||t('profile_unnamed');el.profileSummaryLocale.textContent=t('language_label')+': '+localeName(el.profileLocale.value||lang)}
  function setProfileEditing(on){if(!el.profileForm||!el.profileSummary)return;el.profileForm.hidden=!on;el.profileSummary.hidden=on;if(on)queueMicrotask(()=>el.profileName.focus())}
  ...
  el.editProfileButton.addEventListener('click',()=>{clearStatus();setProfileEditing(true)});
  el.cancelProfileButton.addEventListener('click',async()=>{clearStatus();const {data}=await client.auth.getSession();if(data.session?.user)await loadProfile(data.session.user);else setProfileEditing(false)});

  el.profileForm.addEventListener('submit',async e=>{
    e.preventDefault();clearStatus();try{const {data,error}=await client.auth.getSession();if(error)throw error;const user=data.session?.user;if(!user)throw new Error('Session not found.');const result=await client.from('runlu_profiles').update({display_name:el.profileName.value.trim()||null,locale:el.profileLocale.value}).eq('user_id',user.id);if(result.error)throw result.error;applyLanguage(el.profileLocale.value);syncProfileSummary();setProfileEditing(false);showStatus(t('profile_saved'))}catch(e2){showStatus(e2?.message||t('generic_error'),true)}
  });
