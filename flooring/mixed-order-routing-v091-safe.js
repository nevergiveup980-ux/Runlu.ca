/* RUNLU Deerfoot Flooring OS · V0.3.91 Mixed Order Line Routing · Safe Reintroduction
...
async function pagedWarehouseRecords(datasetKeys,{contains=null,pageSize=250,maxRows=5000}={}){
  const c=await client(),out=[];
  for(let from=0;from<maxRows;from+=pageSize){
    let q=c.from('warehouse_records').select('dataset_key,record_id,payload,updated_at').in('dataset_key',datasetKeys).is('deleted_at',null).order('updated_at',{ascending:false}).range(from,Math.min(from+pageSize-1,maxRows-1));
    if(contains)q=q.contains('payload',contains);
    const r=await q;if(r.error)throw r.error;
    const rows=Array.isArray(r.data)?r.data:[];out.push(...rows);
...
async function refreshCloud(){
  ...
  const rows=await pagedWarehouseRecords(['runlu_product_master_v21']);
  masters=new Map(rows.map(r=>[str(r.record_id||r.payload?.id),r.payload||{}]));
  try{await window.RUNLUSmartInventoryPickerV072?.refresh?.(false)}catch(_){}
}
async function existingFor(po){
  const rows=await pagedWarehouseRecords(['runlu_operations_log_v52','runlu_cutting_log_v52'],{contains:{po:String(po)}});
  existing=rows.map(r=>r.payload||{});return existing;
}
