/* RUNLU Flooring OS · People TO Call Sales Review Routing V0.3.95r2 · Safe Reintroduction
...
function attachPeopleObserver(){
  const list=by('r66peopleList');if(!list)return false;if(window.__r95peopleObserver&&window.__r95peopleObserved===list)return true;
  try{window.__r95peopleObserver?.disconnect?.()}catch(_){}
  let timer=0;const ob=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(()=>{patchPeopleRows();renderPickupReview()},20)});ob.observe(list,{childList:true});window.__r95peopleObserver=ob;window.__r95peopleObserved=list;return true;
}
function install(){
  ensureStyle();bind();
  [80,320,900,1800].forEach(ms=>setTimeout(()=>{refreshAll();attachPeopleObserver()},ms));
}
window.RUNLUPeopleToCallReviewV095={version:'0.3.95r2-safe',install,refresh:refreshAll,route,poStatusImmutable:true,atomicReviewCommit:true,networkWrites:false,jobRoutes:['People TO Call','Pick Up','Active']};
