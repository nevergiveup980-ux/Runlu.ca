/* RUNLU Deerfoot Flooring OS · V0.3.99 Dispatch Action Required Preview
...
(function(root){
'use strict';
if(root.__RUNLU_WEEK_SCHEDULE_V099__)return;
...
function actionRequired(datasets,tasks,date,limit=5){
  ...
  dispatch.materialAttention.forEach(x=>{
    ...
    pushJob(x.job,score,reason,'material');
  });
  dispatch.gaps.forEach(g=>{
    ...
    pushJob(job,score,`${today?'Today':'Near-term'}: missing ${g.missing.join(' + ')}`,'schedule-gap');
  });
  items.forEach(x=>{
    if(x.kind==='job'){
