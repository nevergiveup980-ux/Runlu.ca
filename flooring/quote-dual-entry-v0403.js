...
function normalizeQuote(q={},job={}){
  ...
  return {
    ...
    gstRate:num(raw.gstRate)==0&&raw.gstRate!==0?GST_DEFAULT:num(raw.gstRate??GST_DEFAULT),
...
function bindInputs(scope){
  ...
  scope.querySelectorAll('[data-q403-line]').forEach(x=>x.oninput=()=>{draft=updateLine(draft,x.dataset.q403Line,x.dataset.id,x.dataset.key,x.value);markDirty();renderSummaryAndPreview();if(mode==='table'&&['qty','unitPrice'].includes(x.dataset.key))renderTable()});
