...
async function pagedDataset(datasetKey,{pageSize=250,maxRows=5000}={}){
  const c=await client(),out=[];
  for(let from=0;from<maxRows;from+=pageSize){
    const q=await c.from('warehouse_records').select('record_id,payload,updated_at').eq('dataset_key',datasetKey).is('deleted_at',null).order('updated_at',{ascending:false}).range(from,Math.min(from+pageSize-1,maxRows-1));
    if(q.error)throw q.error;
    const rows=Array.isArray(q.data)?q.data:[];out.push(...rows);
...
async function refresh(manual=false){
  ...
  try{
    ...
    const [ir,or,rr]=await Promise.all([
      pagedDataset(INVENTORY_DATASET,{maxRows:5000}),
      pagedDataset(OPERATIONS_DATASET,{maxRows:5000}),
      pagedDataset(REGISTRY_DATASET,{maxRows:5000})
    ]);
    inventory=ir;operations=or;registry=rr;lastSync=stamp();setState(`LIVE · ${inventory.length} ROLLS`);render();if(manual)setMessage(`RC Tracking refreshed at ${lastSync}.`);
...
window.RUNLUCarpetRCTrackingV092={version:'0.3.92-safe',open,refresh,combined,pagedDataset,readOnlyInventory:true,autoGenerateRC:false,registryDataset:REGISTRY_DATASET};
