...
(function(){
  ...
  function knownNumbers(){
    ...
    const a=auth(),assigned=Object.values(a.assignments||{}).map(parseNo).filter(Boolean);
    return {real,demo,assigned,last:parseNo(a.lastIssued)};
  }
  function currentFloor(){const k=knownNumbers(),base=k.real.length?k.real:k.demo;return Math.max(0,k.last,...base,...k.assigned)}
  ...
  function allocateNext(key,baseline){
    const held=assigned(key);if(held)return held;
    const floor=Math.max(currentFloor(),parseNo(baseline));
    if(!floor)return '';
  ...
  function ensureRegularInvoiceNumber(){
    ...
    let n='';const floor=currentFloor();
  ...
  function wrapRegularInvoice(){
    ...
    if(typeof basePrepare==='function'){
      window.prepareInvoice=function(){ensureRegularInvoiceNumber();return basePrepare.apply(this,arguments)};
    }
    if(typeof baseSave==='function'){
      window.saveJob=function(){
        ...
        if(j){
          if(blankBefore&&String(j.invoiceNumber||'')===String(j.jobNumber||'')){
            j.invoiceNumber='';persistActiveJob(j);if(field)field.value='';
  ...
  function claimInvoiceNumber(j){
    ...
    return allocateNext(key,j.invoiceNumber||j.jobNumber);
  }
  function injectClaimNumber(win,n,j){
    try{
      ...
      if(msg){
        ...
        msg.innerHTML=n
          ?'<b>Invoice # C'+n+'</b> · assigned from the shared Regular / Claim invoice sequence.'+(caseNo?' &nbsp; Claim case: <b>'+String(caseNo).replace(/[<>]/g,'')+'</b>.':'')+(original?' &nbsp; Previous invoice remains <b>'+original+'</b>.':'')
  ...
  function claimEntry(){
    ...
    box.innerHTML='<b style="display:block;margin-bottom:5px;color:#173d30">Deerfoot Claim Invoice</b><span>White legacy Claim Invoice · Job-bound · shared Invoice # sequence with Regular Invoice.</span>';
  ...
  function boot(){wrapRegularInvoice();installEntry();claimEntry();commandEntries();markVersion();window.RUNLUInvoiceNumbers={ensureRegular:ensureRegularInvoiceNumber,claimForActive:()=>claimInvoiceNumber(activeJob()),lastIssued:()=>currentFloor()}}
