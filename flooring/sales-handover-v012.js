/* RUNLU Deerfoot Flooring OS · Sales Handover / Customer Ownership V0.1.2
   Customer ownership changes are separated from historical sales attribution.
   Completed/Closed/Cancelled Jobs keep their original Sales Rep. Open Jobs may transfer.
   Issued/open POs keep original salesRep and receive responsibleSalesRep for handover continuity.
   Validation build: browser-local storage, matching the current Flooring Job data model. */
(function(){
  'use strict';
  const SETTINGS_KEY='runlu_deerfoot_sales_pricing_settings_v1';
  const OWNERSHIP_KEY='runlu_deerfoot_customer_ownership_v1';
  const PO_KEY='runlu_deerfoot_supplier_orders_v1';
  const CLOSED_JOB=new Set(['Completed','Closed','Cancelled']);
  const CLOSED_PO=new Set(['Completed','Cancelled']);
  ...
  function dateOfJob(j){return String(j?.date||j?.invoiceDate||j?.dateRequired||'').slice(0,10)}
  function loadJSON(k,fallback){try{const v=JSON.parse(localStorage.getItem(k)||'null');return v??fallback}catch(_){return fallback}}
  function saveJSON(k,v){localStorage.setItem(k,JSON.stringify(v))}
  function settings(){const s=loadJSON(SETTINGS_KEY,{reps:[],inactiveReps:[]});s.reps=Array.isArray(s.reps)?s.reps:[];s.inactiveReps=Array.isArray(s.inactiveReps)?s.inactiveReps:[];return s}
  function activeReps(){const s=settings(),inactive=new Set(s.inactiveReps.map(norm));return [...new Set(s.reps.filter(r=>r&&String(r).toUpperCase()!=='DF').filter(r=>!inactive.has(norm(r))))].sort(alpha)}
  function jobsSafe(){try{return Array.isArray(jobs)?jobs:[]}catch(_){return []}}
  function poSafe(){const p=loadJSON(PO_KEY,[]);return Array.isArray(p)?p:[]}
