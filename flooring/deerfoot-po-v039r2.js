/* RUNLU Deerfoot Flooring OS · Deerfoot PO Renderer V0.3.39R2 */
(function(){
  'use strict';
  ...
  function fitMobile(){
    ...
    if(window.matchMedia('(max-width:760px)').matches){
      ...
      sheet.style.transform=`scale(${scale})`;sheet.style.transformOrigin='top left';holder.style.height=(sheet.offsetHeight*scale+18)+'px';holder.style.width='100%';
  ...
  function render(){
    ...
    set('poDate',d.orderDate||'—');set('poNumber',po);set('poNumberBottom',po);set('supplierName',d.supplier||'—');set('jobNumber',d.jobNumber||'—');set('customerName',d.customerName||'—');
    set('status',d.status||'Draft');set('salesRep',d.salesRep||'—');set('pickup',d.requestedDate||'—');set('fulfillment',d.fulfillment||'Pickup');set('expected',d.expectedDate||'—');set('purchaseType',d.purchaseType||'Job-specific');
    ...
    const sub=items.reduce((s,x)=>s+lineTotal(x),0);set('subTotal',money(sub));set('poTotal',money(sub));set('copyTag',(d.status==='Draft'?'DRAFT PO PREVIEW':'PURCHASE ORDER')+' · V0.3.39R2');fitMobile();
  }
