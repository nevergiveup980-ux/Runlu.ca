/* RUNLU Flooring OS · Carpet Line RC Link V0.3.93 · Safe Reintroduction
...
function connectObserver(){
  ...
  const editor=by('itemsEditor');if(!editor)return;
  observer=new MutationObserver(()=>{const now=Date.now();if(now-lastDecoratedAt>30)setTimeout(decorate,0)});
  observer.observe(editor,{childList:true,subtree:true})
}
function install(){
  ensureStyle();patchBaseFunctions();decorate();connectObserver();
  document.addEventListener('click',e=>{const b=e.target.closest?.('button');if(!b)return;if(b.dataset?.page==='jobs'||/Jobs\s*&?\s*Orders|Jobs/i.test(b.textContent||''))setTimeout(()=>{decorate();connectObserver();refreshRCData()},120)},true);
  window.addEventListener('focus',()=>{if(by('jobs')?.classList?.contains('active'))refreshRCData()});
  [180,650,1600].forEach(ms=>setTimeout(()=>{decorate();ensureDatalist();renderAllStrips();connectObserver()},ms));
  if(!combined().length)setTimeout(refreshRCData,700);
}
window.RUNLUCarpetLineRCV093={version:'0.3.93-safe',decorate,refresh:refreshRCData,parseCuts,planning,cutAllowanceFeet:CUT_ALLOWANCE_FT,maxDatalistOptions:MAX_DATALIST_OPTIONS,readOnlyInventory:true};
