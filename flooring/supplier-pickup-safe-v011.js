/* RUNLU Deerfoot Flooring OS · Supplier Pickup Safe V0.1.3
   ...
   Pickup / receiving metadata is now read from each PO record, with legacy metadata fallback.
...
(function(){
  ...
  function scheduleRows(){
    ...
    return records.filter(x=>x.poNumber&&x.status!=='Draft').map(po=>{
      ...
      return {...po,
        ...
        requestedDate:po.requestedDate||legacy.requestedDate||po.expectedDate||'',
  ...
  function boot(){ensurePage();ensureNav();render();window.runluPickupSafeRender=render;window.openSupplierPickupSafe=openPickup;window.addEventListener('storage',ev=>{if([PO_STORE,META_STORE].includes(ev.key))render()});}
