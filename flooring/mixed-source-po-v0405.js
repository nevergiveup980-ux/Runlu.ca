/* RUNLU Deerfoot Flooring OS · V0.4.05 Mixed-Source Execution Router Preview
...
async function connectCloud(){
  ...
  try{const c=await client(),r=await c.auth.signInWithPassword({email,password});if(r.error)throw r.error;cloudState='CONNECTED';await refreshCloud()}catch(e){cloudState='ERROR · '+(e?.message||e)}finally{cloudBusy=false;renderCloud()}
}
async function refreshCloud(){
  if(cloudBusy)return;cloudBusy=true;cloudState='SYNCING';renderCloud();
  try{
    const c=await client(),s=(await c.auth.getSession()).data?.session;if(!s){cloudState='SIGN IN REQUIRED';cloudInventory=[];cloudHolds=[];cloudSupplierTasks=[];return}
    const [wr,hr,tr]=await Promise.all([
    ...
    if(wr.error)throw wr.error;if(hr.error)throw hr.error;if(tr.error)throw tr.error;
    cloudInventory=buildCloudInventory(wr.data||[]);cloudHolds=hr.data||[];cloudSupplierTasks=tr.data||[];cloudState='LIVE';
...
async function stageCloudPlans(){
  ...
  try{
    ...
    cloudState='STAGED';await refreshCloud();alert('Cloud work plans staged. Warehouse remains the execution authority.')
...
function renderCloud(){
  ...
  const routing=executionRouting(draft),supplierTask=cloudSupplierTasks.find(t=>String(t.po_number)===String(draft.poNumber||'')),held=routing.inventory.filter(x=>currentCloudHold(x)).length;
  ...
  <div class="ms405cloudActions"><button type="button" id="ms405refresh" ${cloudBusy?'disabled':''}>Refresh Cloud</button><button type="button" id="ms405stage" class="primary" ${cloudBusy||routing.blockers.length?'disabled':''}>Stage Work Plans</button></div>
  ...
  by('ms405refresh')?.addEventListener('click',refreshCloud);by('ms405stage')?.addEventListener('click',stageCloudPlans);by('ms405connect')?.addEventListener('click',connectCloud)
}
function load(){
