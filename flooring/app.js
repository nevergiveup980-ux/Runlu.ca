...
function renderNav(){const n=document.getElementById('nav');n.innerHTML=NAV.map(([id,l])=>`<button data-page="${id}" onclick="go('${id}')">${l}</button>`).join('')}
...
function renderCommand(){const open=jobs.filter(j=>!['Closed','Cancelled'].includes(j.status)).length;const mat=jobs.filter(j=>j.status==='Procurement'||!(j.supplierPO||'').trim()).length;const inst=jobs.filter(j=>['Ready','In Progress'].includes(j.status)||['Scheduled','In Progress'].includes(j.installStatus)).length;const bal=jobs.reduce((s,j)=>s+calc(j).balance,0);stOpen.textContent=open;stMaterial.textContent=mat;stInstall.textContent=inst;stBalance.textContent=money(bal).replace('.00','');const j=active();activeSummary.innerHTML=j?`<b>${j.isDemo?'<span class="tag demoTag">DEMO</span> ':''}${esc(j.jobNumber||'No #')} · ${esc(j.customerName||'Unnamed customer')}</b><br>${esc(j.status||'Draft')} · Required ${esc(j.dateRequired||'—')} · Balance ${money(calc(j).balance)}`:'No active Job.';const items=[];jobs.filter(j=>!['Closed','Cancelled'].includes(j.status)).slice(0,6).forEach(j=>{if(j.status==='Procurement'||!(j.supplierPO||'').trim())items.push([j,'Material / PO needs attention']);else if(j.dateRequired)items.push([j,'Required '+j.dateRequired]);else if(calc(j).balance>0)items.push([j,'Balance '+money(calc(j).balance)])});attention.innerHTML=items.length?items.map(([j,t])=>`<div class="attention"><b>${j.isDemo?'<span class="tag demoTag">DEMO</span> ':''}${esc(j.jobNumber)} · ${esc(j.customerName)}</b><span>${esc(t)}</span></div>`).join(''):'<div class="muted">No open attention items.</div>'}
function renderJobs(){const el=document.getElementById('jobList');el.innerHTML=jobs.length?jobs.map(j=>`<div class="jobRow"><div><b>${j.isDemo?'<span class="tag demoTag">DEMO</span> ':''}${esc(j.jobNumber||'No #')} · ${esc(j.customerName||'Unnamed customer')}</b><small>${esc(j.status||'Draft')} · ${esc(j.dateRequired||'No required date')} · ${money(calc(j).balance)} due</small></div><button class="action" onclick="selectJob('${j.id}')">Open</button></div>`).join(''):'<div class="muted">No Jobs yet.</div>'}
...
function saveJob(){const j=active();if(!j)return;['jobNumber','invoiceNumber','customerName','clerk','soldToAddress','shipToName','shipToAddress','email','cell','dateRequired','status','pickup','delivery','customerPO','supplierPO','notes'].forEach(k=>j[k]=byId(k).value.trim());j.items=editingItems;j.invoiceNumber=j.invoiceNumber||j.jobNumber;j.invoiceDate=j.invoiceDate||new Date().toISOString().slice(0,10);j.isDemo=false;saveStore();renderAll();alert('Job saved.')}
function renderPurchasing(){const j=active();if(!j){poSummary.textContent='Select a Job.';return}poSummary.innerHTML=`<b>${j.isDemo?'<span class="tag demoTag">DEMO</span> ':''}${esc(j.jobNumber)} · ${esc(j.customerName)}</b><br>Supplier PO: ${esc(j.supplierPO||'Not assigned')} · Customer PO: ${esc(j.customerPO||'—')} · Status: ${esc(j.status)}`;poItems.innerHTML=(j.items||[]).map(x=>`<div class="jobRow"><div><b>${esc(x.style||'Item')} · ${esc(x.colour||'')}</b><small>${esc(x.qty||'')} · ${esc(x.supplier||'Supplier not set')}</small></div><span class="tag">${money(x.total||0)}</span></div>`).join('')||'<div class="muted">No items.</div>'}
...
function renderWarehouse(){const j=active();handoff.textContent=handoffText(j);refreshWarehouse()}
...
function refreshWarehouse(){warehouseFrame.src=warehouseUrl()}
...
function loadInstall(){const j=active();if(!j)return;['installer','installDate','installStatus','installNotes'].forEach(k=>byId(k).value=j[k]||'')}
function saveInstall(){const j=active();if(!j)return;['installer','installDate','installStatus','installNotes'].forEach(k=>j[k]=byId(k).value);saveStore();renderAll();alert('Installation saved.')}
function loadAccounting(){const j=active();if(!j)return;deliveryCharge.value=Number(j.deliveryCharge||0);depositPaid.value=Number(j.depositPaid||0);paymentMethod.value=j.paymentMethod||'';renderAccounting()}
function renderAccounting(){const j=active();if(!j)return;const c=calc(j);aSub.textContent=money(c.subtotal);aGst.textContent=money(c.gst);aTotal.textContent=money(c.total);aBalance.textContent=money(c.balance)}
function saveAccounting(){const j=active();if(!j)return;j.deliveryCharge=Number(deliveryCharge.value||0);j.depositPaid=Number(depositPaid.value||0);j.paymentMethod=paymentMethod.value;saveStore();renderAccounting();renderAll();alert('Accounting saved.')}
