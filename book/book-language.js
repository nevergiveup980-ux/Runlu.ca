(()=>{
  ...
  const ALT={
    en:'Cover of The Wind Beyond the Walls',
    zh:'《庐外有风》封面',
  ...
  function ensureCover(){
    ...
    if(!hero)return null;
    let panel=hero.querySelector('.book-cover-panel');
    if(panel)return panel;

    panel=document.createElement('a');
    panel.className='book-cover-panel';
    panel.href='prologue.html';
    ...
    const status=hero.querySelector('.book-status');
    hero.insertBefore(panel,status||null);

    if(!document.getElementById('book-cover-style')){
      const style=document.createElement('style');
      style.id='book-cover-style';
      style.textContent=`
        ...
        @media(min-width:761px){
          .book-hero{grid-template-columns:minmax(0,1fr) minmax(190px,240px);gap:42px 58px;align-items:start}
          .book-hero>div:first-child{grid-column:1;grid-row:1 / span 2}
          .book-cover-panel{grid-column:2;grid-row:1}
          .book-status{grid-column:2;grid-row:2;margin-top:0}
        }
        @media(max-width:760px){
          .book-cover-panel{width:min(72vw,260px);margin:2px auto 6px}
        }
      `;
