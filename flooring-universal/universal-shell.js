...
$('saveSetup').addEventListener('click',()=>{if(window.RUNLUUniversalStartupGuard&&!window.RUNLUUniversalStartupGuard.canProceed())return alert('Startup Recovery Guard has paused workspace changes. Open Support & Recovery first.');const legal=$('legalName').value.trim(),display=$('displayName').value.trim()||legal,loc=$('locationName').value.trim();if(!legal||!loc)return alert('Enter the legal company name and first location.');const org='org-'+slug(legal),lid='loc-'+slug(loc);const c=window.RUNLUFlooringUniversal.createConfig({company:{organizationId:org,legalName:legal,displayName:display,abbreviation:$('abbreviation').value.trim().toUpperCase(),country:$('country').value,region:$('region').value.trim(),currency:$('currency').value},location:{locationId:lid,name:loc,type:$('locationType').value}});write(c);window.RUNLUUniversalDataVersion?.migrate?.();render(c);setTimeout(()=>window.RUNLUUniversalDeviceReady?.renderMini?.(),500)});
async function boot(){
 const guard=await window.RUNLUUniversalStartupGuard?.ready?.();
 const current=read();fill(current);
 if(current&&guard?.blocked){
  $('command').hidden=true;$('saveSetup').disabled=true;$('setupState').className='warn';$('setupState').textContent='Startup protection is active. Review Support & Recovery before normal workspace operations.';window.RUNLUUniversalStartupGuard?.renderBanner?.();return;
 }
 if(current)window.RUNLUUniversalDataVersion?.migrate?.();render(current);window.RUNLUUniversalStartupGuard?.renderBanner?.();
}
boot();
})();