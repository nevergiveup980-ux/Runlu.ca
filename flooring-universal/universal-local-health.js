...
(function(){
'use strict';
const SNAP='runlu_flooring_universal_u2_recovery_points',PREFIX='runlu_flooring_universal_',MAX=12,WS='runlu_flooring_universal_u0_workspace';
...
function capture(reason,meta){
 ...
 (adapter.rawKeys?.()||[]).filter(k=>k.startsWith(PREFIX)&&k!==SNAP&&k!=='runlu_flooring_universal_data_backend').forEach(k=>payload[k]=adapter.read(k,null));
...
function restore(id){
 ...
 (adapter.rawKeys?.()||[]).filter(k=>k.startsWith(PREFIX)&&k!==SNAP&&k!=='runlu_flooring_universal_data_backend').forEach(k=>adapter.remove(k));
 Object.entries(p.payload||{}).forEach(([k,v])=>{if(k.startsWith(PREFIX)&&k!==SNAP&&k!=='runlu_flooring_universal_data_backend')adapter.write(k,v)});
