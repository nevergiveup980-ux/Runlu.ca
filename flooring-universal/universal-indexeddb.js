...
async function recoverMissing(){
 ...
 for(const r of records){if(!r.key?.startsWith(PREFIX)||r.key===BACKEND||r.key===SNAP||r.key===GUARD)continue;if(adapter.raw(r.key)===null){adapter.write(r.key,r.value);restored++}else skipped++}
...
function init(){
 if(!supported())return;
 window.RUNLUUniversalData?.onMutation?.(e=>{if(e.adapter!=='local'||!e.key?.startsWith(PREFIX)||e.key===BACKEND||e.key===SNAP||e.key===GUARD)return;e.type==='remove'?remove(e.key):put(e.key,e.value)});
