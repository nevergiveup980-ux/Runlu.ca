...
(function(){
'use strict';
const DB='runlu-flooring-universal-local',DBV=1,STORE='records',PREFIX='runlu_flooring_universal_',BACKEND='runlu_flooring_universal_data_backend';
...
async function put(key,value){
 if(!key?.startsWith(PREFIX)||key===BACKEND)return;
 pending++;
...
async function seed(){
 ...
 const adapter=window.RUNLUUniversalData.current(),keys=(adapter.rawKeys?.()||[]).filter(k=>k.startsWith(PREFIX)&&k!==BACKEND);
...
async function recoverMissing(){
 ...
 for(const r of records){if(!r.key?.startsWith(PREFIX)||r.key===BACKEND)continue;if(adapter.raw(r.key)===null){adapter.write(r.key,r.value);restored++}else skipped++}
...
async function replaceLocalFromMirror(){
 ...
 records.forEach(r=>{if(r.key?.startsWith(PREFIX)&&r.key!==BACKEND)adapter.write(r.key,r.value)});
...
function init(){
 if(!supported())return;
 window.RUNLUUniversalData?.onMutation?.(e=>{if(e.adapter!=='local'||!e.key?.startsWith(PREFIX)||e.key===BACKEND)return;e.type==='remove'?remove(e.key):put(e.key,e.value)});
