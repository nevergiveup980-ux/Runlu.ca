...
(function(){
'use strict';
const PREFIX='runlu_flooring_universal_',BACKEND='runlu_flooring_universal_data_backend',SNAP='runlu_flooring_universal_u2_recovery_points',FORMAT='runlu-flooring-universal-backup',VERSION=1,GUARD='runlu_flooring_universal_u2_startup_guard',JOURNAL='runlu_flooring_universal_u2_crash_journal';
const RESERVED=new Set([BACKEND,SNAP,GUARD,JOURNAL]);
...
function collect(){
 ...
 keys.filter(k=>k.startsWith(PREFIX)&&k!==BACKEND&&k!==SNAP&&k!==GUARD&&k!==JOURNAL).forEach(k=>{data[k]=adapter.read(k,null)});
