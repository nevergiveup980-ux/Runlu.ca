...
function run(){
 const org=read(WS,null)?.company?.organizationId||'',jobs=window.RUNLUUniversalSales?.jobs()||[],pos=window.RUNLUUniversalPO?.pos()||[],inb=window.RUNLUUniversalInbound?.tasks()||[],inst=window.RUNLUUniversalInstallation?.rows()||[],bills=window.RUNLUUniversalBilling?.rows()||[],acct=window.RUNLUUniversalAccounting?.rows()||[];
 ...
 return {org,counts:{jobs:jobs.length,pos:pos.length,inbound:inb.length,installations:inst.length,customerInvoices:bills.length,supplierAccounting:acct.length},issues};
}
