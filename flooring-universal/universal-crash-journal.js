...
function inspect(){const s=state();return {active:s.active.length,history:s.history.length,pending:s.active.map(x=>({id:x.id,type:x.type,action:x.action,startedAt:x.startedAt,meta:x.meta||{}}))}}
...
window.RUNLUUniversalCrashJournal=Object.freeze({STORE,begin,commit,abort,pending,history,inspect,render});
})();