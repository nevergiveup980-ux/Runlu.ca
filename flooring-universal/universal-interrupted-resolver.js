...
function auditMatchWith(tx,reader){
 const events=reader(KEYS.audit),m=tx.meta||{};
...
function classifyWith(tx,reader){
 ...
 if(tx.type==='Supplier PO'){
  ...
  record=find(reader(KEYS.inbound),m.inboundId);applied=!!record?.receivedAt;notApplied=!!record&&!record.receivedAt;evidence.push(record?.receivedAt?'Receiving timestamp exists; status '+record.status+'.':record?'Receiving record has no received timestamp.':'Receiving record not found; classification remains uncertain.');
 }else if(tx.type==='Installation'){
  record=find(reader(KEYS.install),m.installationId);applied=record?.status==='Completed'&&!!record.completedAt;notApplied=!!record&&record.status!=='Completed';evidence.push(record?'Installation status: '+record.status+(record.completedAt?' with completion timestamp.':'.'):'Installation record not found; classification remains uncertain.');
...
function classifyFixture(tx,fixture){const reader=key=>structuredClone(fixture?.[key]||[]);return classifyWith(tx,reader)}
...
window.RUNLUUniversalInterruptedResolver=Object.freeze({classify,classifyFixture,scan,acknowledge,render,auditMatch,KEYS});
})();