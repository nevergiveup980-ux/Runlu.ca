...
function classify(tx){
 ...
 if(tx.type==='Supplier PO'){
  ...
  record=find(arr(KEYS.inbound),m.inboundId);applied=!!record?.receivedAt;notApplied=!!record&&!record.receivedAt;evidence.push(record?.receivedAt?'Receiving timestamp exists; status '+record.status+'.':record?'Receiving record has no received timestamp.':'Receiving record not found; classification remains uncertain.');
 }else if(tx.type==='Installation'){
  record=find(arr(KEYS.install),m.installationId);applied=record?.status==='Completed'&&!!record.completedAt;notApplied=!!record&&record.status!=='Completed';evidence.push(record?'Installation status: '+record.status+(record.completedAt?' with completion timestamp.':'.'):'Installation record not found; classification remains uncertain.');
 }else if(tx.type==='Customer Invoice'){
  record=find(arr(KEYS.invoice),m.invoiceId);applied=!!record?.invoiceNumber&&record.status!=='Draft';notApplied=!!record&&record.status==='Draft';evidence.push(record?'Invoice status: '+record.status+(record.invoiceNumber?' · #'+record.invoiceNumber:' · no invoice number')+'.':'Invoice record not found; classification remains uncertain.');
  ...
  record=find(arr(KEYS.accounting),m.accountingId);applied=record?.status==='Paid'&&!!record.paidAt;notApplied=!!record&&record.status!=='Paid';evidence.push(record?'Supplier accounting status: '+record.status+(record.paidAt?' with paid timestamp.':'.'):'Supplier accounting record not found; classification remains uncertain.');
 }
...
window.RUNLUUniversalInterruptedResolver=Object.freeze({classify,scan,acknowledge,render,auditMatch,KEYS});
})();