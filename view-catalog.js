(()=>{
  const entries=[
    ...
    {n:'014',href:'view-014-ai-biodesign.html',date:'5 SEP 2026',en:'When AI begins designing biology',zh:'当 AI 开始设计生物学',fr:'Quand l’IA commence à concevoir le vivant',es:'Cuando la IA empieza a diseñar biología'},
    {n:'015',href:'view-015-ai-fusion-control.html',date:'7 SEP 2026',en:'When AI acts faster than a human can react',zh:'当 AI 的动作快到人类来不及反应',fr:'Quand l’IA agit plus vite qu’un humain',es:'Cuando la IA actúa más rápido que una persona'}
  ];
  const makeLink=(entry,latest=false)=>{const a=document.createElement('a');a.className=latest?'catalog-card latest-card':'catalog-card';a.href=entry.href;a.dataset.runluCatalog='true';const number=document.createElement('span');number.className='catalog-no';number.textContent=entry.n;const title=document.createElement('span');title.className='catalog-title';title.dataset.en=entry.en;title.dataset.zh=entry.zh;title.dataset.fr=entry.fr;title.dataset.es=entry.es;title.textContent=entry.en;a.append(number,title);if(entry.date){const date=document.createElement('span');date.className='catalog-date';date.textContent=entry.date;a.appendChild(date)}const arrow=document.createElement('span');arrow.className='catalog-arrow';arrow.textContent='→';a.appendChild(arrow);return a};
  function render(){const catalog=document.getElementById('viewCatalog');if(catalog&&!catalog.dataset.ready){entries.slice().reverse().forEach(e=>catalog.appendChild(makeLink(e)));catalog.dataset.ready='true'}const latest=document.getElementById('viewLatest');if(latest&&!latest.dataset.ready){latest.appendChild(makeLink(entries[entries.length-1],true));latest.dataset.ready='true'}if(window.RUNLULanguage)window.RUNLULanguage.apply(window.RUNLULanguage.get())}
  window.RUNLUViewCatalog={entries:[...entries]};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',render,{once:true});else render()
})();